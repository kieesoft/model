...
class User extends MyService{
    ...
    public function getUserList($page=1,$rows=10,$username='%',$name='%',$school=''){
        ...
        $data=$this->query->table('users')->join('teachers','teachers.teacherno=users.teacherno')
            ...
            remoteip,logintime,createdate,enteryear,rtrim(sexcode.name) sexname ')
    ...
    {
        if ($teacherno == '')
            throw new \Think\Exception(MyAccess::getErrorMessage(MyException::PARAM_NOT_CORRECT), MyException::PARAM_NOT_CORRECT);
        else {
            $condition = null;
    ...
    public function getUserRole($username='')
    {
        ...
        if(is_array($data)&&count($data)>0){
            ...
            throw new \Think\Exception(MyAccess::getErrorMessage(MyException::USER_NOT_EXISTS).$username,MyException::USER_NOT_EXISTS);
        return $result;
    }
    ...
    public function changeSelfPassword($oldPassword,$newPassword){
        ...
        $result= $this->query->table('users')->where($condition)->find();
